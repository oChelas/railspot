...
import { ArrowLeft, MapPin, Bell, Navigation, Send, User, Car, Footprints, CornerUpRight, XCircle, ExternalLink, Edit, Trash2 } from 'lucide-react';
...
function StationDetails({ station, onBack }) {
  ...
  // --- NOVA FUNÇÃO: ELIMINAR COMENTÁRIO (RF19) ---
  const handleDeleteReview = async (reviewId) => {
    const isConfirmed = window.confirm("Tem a certeza que quer apagar este comentário?");
    ...
    try {
      ...
      if (res.ok) {
        // Atualiza a lista no ecrã imediatamente, filtrando o comentário apagado
        setReviews(reviews.filter(r => r.id !== reviewId));
      } else {
        alert("Erro ao eliminar o comentário.");
      }
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteStation = async () => {
    const isConfirmed = window.confirm(`Tem a certeza absoluta que pretende eliminar a ${station.name}? Esta ação é irreversível.`);
    
    if (isConfirmed) {
      const token = localStorage.getItem('token');
      try {
        const res = await fetch(`http://localhost:5000/api/stations/${station.id}`, {
          method: 'DELETE',
          headers: { 'x-auth-token': token }
        });

        if (res.ok) {
          alert('Estação eliminada com sucesso!');
          onBack(); 
          window.location.reload(); 
        } else {
          const errorData = await res.json();
          alert('Erro: ' + (errorData.error || 'Não foi possível eliminar.'));
        }
      } catch (error) {
        console.error(error);
        alert('Erro de conexão ao servidor.');
    ...
    if ("geolocation" in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          else if (error.code === 3) alert("O GPS demorou demasiado tempo.");
          ...
          {isNavigating ? (
            userLocation ? (
              ...
              <div className="w-full h-full bg-slate-900 flex flex-col items-center justify-center text-white p-6 text-center animate-fade-in z-50">
                  ...
                  {gpsError && <div className="mb-6 bg-red-500/20 text-red-200 p-3 rounded-lg text-sm border border-red-500/50 flex items-center gap-2"><XCircle size={16} /> {gpsError}</div>}
                  <button onClick={stopNavigation} className="bg-white/10 hover:bg-white/20 px-6 py-2 rounded-full text-sm font-medium transition-colors border border-white/10">Cancelar</button>
              </div>
            )
          ) : (
            <>
              ...
              {user && user.is_admin && (
                  <div className="absolute top-4 right-4 z-[400] flex gap-2">
                    <button 
                      onClick={() => navigate(`/admin/edit/${station.id}`)}
                      className="bg-blue-600 p-3 rounded-full shadow-xl text-white hover:bg-blue-700 transition-transform hover:scale-110"
                      title="Editar Fotografia e Detalhes"
                    >
                      <Edit size={24} />
                    </button>
                    <button 
                      onClick={handleDeleteStation}
                      className="bg-red-600 p-3 rounded-full shadow-xl text-white hover:bg-red-700 transition-transform hover:scale-110"
                      title="Eliminar Estação"
                    >
                      <Trash2 size={24} />
                    </button>
                  </div>
                            ...
                            <h3 className="font-bold mb-4 flex items-center gap-2">💬 Comentários <span className="text-gray-400 text-sm font-normal">({reviews.length})</span></h3>
                             {user ? (
                                <form onSubmit={handleSubmitReview} className="flex gap-2 mb-4">
                                    ...
                                    <button disabled={submitting} className="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 disabled:opacity-50 transition-colors"><Send size={18}/></button>
                                </form>
                                ...
                                {reviews.map(r => (
                                    <div key={r.id} className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 group relative">
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            {/* ZONA DE MODERAÇÃO DE COMENTÁRIOS */}
                                            {user && user.is_admin && (
                                                <button 
